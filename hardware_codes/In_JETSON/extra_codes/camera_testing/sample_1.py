...
while True:
    for cam_id, cam in cameras.items():
        ...
        if ret:
            ...
            cv.putText(undistorted_img, fps_text, (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv.imshow(f"Camera {cam_id}", undistorted_img)
            cv.imwrite(f"images/{cam_id}.png", undistorted_img)  # Save each frame as an undistorted image

    if cv.waitKey(1) == ord('q'):  # Corrected condition to exit loop
        break
